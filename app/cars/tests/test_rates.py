...
class TestRate(TestBase):
    ...
    def test_add_rate_fail_max(self):
        car = models.Car.objects.create(**self.CAR_1)
        rate = self.client.post(RATE_URL, {'car_id': car.id, 'rating': 6})
        self.assertEqual(rate.status_code, 400)
        self.assertEqual(
